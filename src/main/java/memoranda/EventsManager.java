...
/*			_root.addNamespaceDeclaration("jnevents", NS_JNEVENTS);
	...
	public static void createSticker(String text, int prior) {
		final Element el = new Element("sticker");
	...
	public static Map getStickers() {
		...
		final Elements els = _root.getChildElements("sticker");
		for (int i = 0; i < els.size(); i++) {
			final Element se = els.get(i);
	...
	public static Collection getEventsForDate(CalendarDate date) {
		...
		final Collection r = getRepeatableEventsForDate(date);
		if (r.size() > 0) {
            v.addAll(r);
        }
	...
	public static Collection getRepeatableEvents() {
		final Vector v = new Vector();
		final Element rep = _root.getFirstChildElement("repeatable");
		if (rep == null) {
            return v;
        }
		final Elements els = rep.getChildElements("event");
		for (int i = 0; i < els.size(); i++) {
            v.add(new EventImpl(els.get(i)));
        }
		return v;
		...
		for (int i = 0; i < reps.size(); i++) {
			...
			if (date.inPeriod(ev.getStartDate(), ev.getEndDate())) {
				if (ev.getRepeat() == REPEAT_DAILY) {
					final int n = date.getCalendar().get(Calendar.DAY_OF_YEAR);
					final int ns =
	...
	private static Year createYear(int y) {
		final Element el = new Element("year");
	...
	private static Year getYear(int y) {
		final Elements yrs = _root.getChildElements("year");
		final String yy = new Integer(y).toString();
	...
	private static Day getDay(CalendarDate date) {
		final Year y = getYear(date.getYear());
		if (y == null) {
            return null;
        }
		final Month m = y.getMonth(date.getMonth());
		if (m == null) {
            return null;
        }
	...
	static class Year {
		...
		public Month getMonth(int m) {
			final Elements ms = yearElement.getChildElements("month");
			final String mm = new Integer(m).toString();
		...
		private Month createMonth(int m) {
			final Element el = new Element("month");
	...
	static class Month {
		...
		public Day getDay(int d) {
			...
			final Elements ds = mElement.getChildElements("day");
			final String dd = new Integer(d).toString();
		...
		private Day createDay(int d) {
			final Element el = new Element("day");
